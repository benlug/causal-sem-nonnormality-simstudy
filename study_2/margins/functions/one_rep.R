one_rep <- function(repNr, condition) {
  ...
  generate_cop_data <- function(treatment) {
    condition$marginsY_theta_ind <- 1 / (condition$load_z * ifelse(treatment, condition$marginsY_theta_wT, condition$marginsY_theta_nT))
    generate_data(
      n = condition$n, 
      mean_y = ifelse(treatment, condition$treat_eff_ystar, 0),
      ...
      marginsY_rate = condition$marginsY_theta_ind,
  ...
  sample_compl_disc[y_cols] <- lapply(sample_compl_disc[y_cols], as.ordered)
  
  sample_compl_disc_cont <- sample_compl_disc %>%
    mutate(across(all_of(y_cols), ~ as.numeric(.) - mean(as.numeric(.))))
  ...
  mm_ML <- generateMeasurementModel(
    ...
    str_replace("c\\(1,1\\)\\*y1", sprintf("c(%s,%s)*y1", condition$load_z, condition$load_z))
  
  run_effectLite <- function(data, mm, estimator) {
    effectLite(y = "eta", x = "x", z = "z",
  ...
  mm_disc <- generateMeasurementModel(
    ...
    str_replace("eta =~ c\\(1,1\\)\\*y1", sprintf("eta =~ c(%s,%s)*y1", condition$load_z, condition$load_z)) %>%
    str_replace("y1 \\| c\\(0,0\\)\\*t1", sprintf("y1 | c(%s,%s)*t1", condition$thresholds[[1]][2], condition$thresholds[[1]][2]))
