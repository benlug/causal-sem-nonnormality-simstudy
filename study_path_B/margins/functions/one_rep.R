one_rep <- function(repNr, condition) {
  ...
  generate_cop_data <- function(group) {
    condition[[paste0("marginsM_theta_", ifelse(group == 1, "nT", "wT"), "_ind")]] <- 
      1 / (condition$loading * condition[[paste0("marginsM_theta_", ifelse(group == 1, "nT", "wT"))]])
    ...
    generate_data(
      ...
      mean_M = condition[[paste0("mean_M_G", group)]],
      Sigma = ifelse(group == 1, Sigma_nT, Sigma_wT),
      margin_dist_M = condition[[paste0("marginsM_", ifelse(group == 1, "nT", "wT"))]],
      marginsM_rate = condition[[paste0("marginsM_theta_", ifelse(group == 1, "nT", "wT"), "_ind")]],
  ...
  mm_disc <- generateMeasurementModel(
    ...
    str_replace("Mpre =~ c\\(1,1\\)\\*m1", sprintf("Mpre =~ c(%s,%s)*m1", condition$loading, condition$loading)) %>%
