one_rep <- function(repNr, condition) {
  ...
  cop_data_nT <- generate_data(n = condition$n, 
                               ...
                               marginsM_rate = condition$marginsM_theta_nT,
  ...
  cop_data_wT <- generate_data(n = condition$n, 
                               ...
                               marginsM_rate = condition$marginsM_theta_wT,
  ...
  mZ := (1000*mZG1 + 1000*mZG2)/2000
  mW := (1000*mWG1 + 1000*mWG2)/2000
  mM := (1000*mMG1 + 1000*mMG2)/2000
  ...
  mm_ML <- generateMeasurementModel(names = "Mpre",
                                    indicators = list("Mpre" = paste0("m", 1:num_m_vars)),
                                    ncells = 2,
                                    model = c("tau-cong"),
  ...
  mm_disc <- generateMeasurementModel(names = "Mpre",
                                      indicators = list("Mpre" = paste0("m", 1:num_m_vars)),
                                      ncells = 2,
                                      model = c("tau-cong-categorical"),
                                      data = as.data.frame(sample_compl_disc))
  mm_disc <- gsub("Mpre =~ c\\(1,1\\)\\*m1", 
                  paste0("Mpre =~ c(", condition$loading, ",", condition$loading, ")*m1"), 
                  mm_disc)
